...
export function subscribe(socket: WebSocket, company: Company): EventChannel<unknown> {
...
function* rootSaga(): Generator<unknown> {
  yield all([
    ...
    takeLatest(listState.types.UNSUBSCRIBE_REQUEST, handleUnsubscribeInstrument),
  ]);
}
